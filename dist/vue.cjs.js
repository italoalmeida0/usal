...
var createUSAL = (config = {}) => {
  return {
    install(app) {
      ...
      app.directive("usal", {
        mounted(el, binding) {
          const value = typeof binding.value === "string" ? binding.value : Object.entries(binding.value || {}).map(([key, val]) => val === true ? key : `${key}-${val}`).join(" ");
...
var useUSAL = (config = {}) => {
  const instance = (0, import_vue.inject)(USAL_KEY) || usal_default.createInstance();
  ...
  (0, import_vue.onUnmounted)(() => {
    if (!(0, import_vue.inject)(USAL_KEY)) {
      instance.destroy();
    }
  });
  return {
...
var Animated = (0, import_vue.defineComponent)({
  name: "Animated",
  props: {
  ...
  setup(props, { slots }) {
    ...
    const dataUsal = (0, import_vue.computed)(() => {
      const parts = [props.animation];
      if (props.direction)
        parts[0] += `-${props.direction}`;
      if (props.duration)
        parts.push(`duration-${props.duration}`);
      if (props.delay)
        parts.push(`delay-${props.delay}`);
      if (props.threshold)
      ...
      if (props.blur)
        parts.push("blur");
      if (props.easing)
        parts.push(props.easing);
      if (props.split)
        parts.push(`split-${props.split}`);
      if (props.splitDelay)
        parts.push(`split-delay-${props.splitDelay}`);
      if (props.count)
        parts.push(`count-[${props.count}]`);
      if (props.text)
        parts.push(`text-${props.text}`);
      return parts.join(" ");
    });
    return () => {
