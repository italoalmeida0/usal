...
var USAL = (() => {
  ...
  const globalInstance2 = createInstance();
  return {
    ...globalInstance2,
    createInstance
  };
...
var getUSAL = () => {
  if (!globalInstance) {
    globalInstance = usal_default.createInstance();
  }
  return globalInstance;
};
var usal = (node, params = "fade") => {
  const instance = getUSAL();
  const getValue = (p) => {
    if (typeof p === "string")
      return p;
    return Object.entries(p).map(([key, val]) => val === true ? key : `${key}-${val}`).join(" ");
  };
