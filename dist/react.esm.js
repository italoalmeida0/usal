...
var USALContext = createContext(null);
var USALProvider = ({ children, config = {} }) => {
  ...
  return /* @__PURE__ */ jsx(USALContext.Provider, { value: instanceRef.current, children });
};
var useUSAL = (config = {}) => {
...
var Animated = forwardRef(({
  ...
  if (direction)
    usualParts[0] += `-${direction}`;
  if (duration)
    usualParts.push(`duration-${duration}`);
  if (delay)
    usualParts.push(`delay-${delay}`);
  if (threshold)
    usualParts.push(`threshold-${threshold}`);
  if (once)
    usualParts.push("once");
  if (blur)
    usualParts.push("blur");
  if (easing)
    usualParts.push(easing);
  if (split)
    usualParts.push(`split-${split}`);
  if (splitDelay)
    usualParts.push(`split-delay-${splitDelay}`);
  if (count)
    usualParts.push(`count-[${count}]`);
  if (text)
    usualParts.push(`text-${text}`);
